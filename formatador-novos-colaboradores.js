...
const readCsv = (formatText) =>
  readFile("novos-colaboradores.csv", "utf-8", (error, text) =>
    error ? new Error(error) : formatText(text)
  )
const createPhoto = (firstName, lastName) =>
  `<td width="150"><img style="width: 120px; height: 160px; display: block; margin-left: auto; margin-right: auto; border-radius: 10%; border: 1px solid; border-color: #ab9b6a;" src="https://arquivos.essentialnutrition.com.br/images/novos-colaboradores/${firstName
    .normalize("NFD")
    ...
    objectPerson.map((el, index) => {
      ...
      const conectorName =
        names[names.length - 2].length === 2
          ...
          : false
      const localName = upperCaseFormatterSector(local.trim()).replace(
        "jovem",
        "Jovem"
      )
      const sectorName = upperCaseFormatterLocal(sector.trim())
      if (index === 0) {
        htmlTextImages += "<tr>"
        htmlTextInfos += "<tr>"
      }
  ...
  const matchRows = () => {
    ...
    const numberOfRows =
      imageRows.length === descriptionRows.length
        ? imageRows.length
        : new Error("Image rows lenght is diferent of description rows lenght.")
